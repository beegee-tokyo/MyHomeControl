...
/**
 * The configuration screen for the {@link Wi_SolarPanel Wi_SolarPanel} AppWidget.
 */
public class WiCo_SolarPanel extends Activity implements AdapterView.OnItemClickListener {
	...
	public void onCreate(Bundle icicle) {
		...
		int uriIndex = tk.giesecke.myhomecontrol.Cl_Utilities.getNotifSounds(this, notifNames, notifUri, false) + 2;
		...
		public void onClick(View v) {
			switch (v.getId()) {
				...
				case R.id.bt_ok:
					...
					final Context context = WiCo_SolarPanel.this;

					/** Access to the shared preferences */
					SharedPreferences mPrefs = getSharedPreferences(tk.giesecke.myhomecontrol.Cl_MyHomeControl.sharedPrefName,0);
					mPrefs.edit().putBoolean(tk.giesecke.myhomecontrol.Cl_MyHomeControl.prefsSolarWidgetSize,isTextLarge).apply();

					if (!notifNameSel.equalsIgnoreCase("")) {
						mPrefs.edit().putString(tk.giesecke.myhomecontrol.Cl_MyHomeControl.prefsSolarWarning,notifUriSel).apply();
					}

					// It is the responsibility of the configuration activity to update the app widget
					/** App widget manager for this widget */
					AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(context);
					Wi_SolarPanel.updateAppWidget(context, appWidgetManager, mAppWidgetId,0f,0f);
					...
					if (mPrefs.getInt(tk.giesecke.myhomecontrol.Cl_MyHomeControl.prefsSolarWidgetNum,0)== 0) {
						mPrefs.edit().putInt(tk.giesecke.myhomecontrol.Cl_MyHomeControl.prefsSolarWidgetNum,1).apply();
					}
					finish();
					break;
			}
