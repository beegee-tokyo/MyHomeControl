 ...
 *
 */
public class Cl_ChartCustomMarkerView extends MarkerView {
	...
	public Cl_ChartCustomMarkerView(Context context) {
	...
	public void refreshContent(Entry e, Highlight highlight) {
		...
		Entry touchSolar = Cl_ChartHelper.solarSeries.get(dataIndex);
		/** Entry with data of consumption at given index */
		Entry touchCons = Cl_ChartHelper.consMSeries.get(dataIndex);
		if (touchCons.getVal() == 0) {
			touchCons = Cl_ChartHelper.consPSeries.get(dataIndex);
		}

		tvMarkerTime.setText(Cl_ChartHelper.timeSeries.get(dataIndex));
		...
		for (int i=0; i<=dataIndex; i++) {
			consAllEntry = Cl_ChartHelper.consMSeries.get(i);
			if (touchCons.getVal() == 0) {
				consAllEntry = Cl_ChartHelper.consPSeries.get(i);
			}
			consAll += consAllEntry.getVal()/60/1000;
			solAllEntry = Cl_ChartHelper.solarSeries.get(i);
...
//		posx = getDeviceWidth(Cl_MyHomeControl.appContext)/2;
