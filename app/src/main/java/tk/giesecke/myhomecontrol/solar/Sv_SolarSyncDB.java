...
//				syncedMonth = "";
				...
				if (!syncDB(Db_Helper.DATABASE_NAME, dbNamesList[0], intentContext)) {
					return;
				}

				// Check if we have already synced the last month
				/** Instance of Db_Helper */
				dbHelper = new Db_Helper(intentContext, Db_Helper.DATABASE_NAME_LAST);
				/** Instance of data base */
				dataBase = dbHelper.getReadableDatabase();
				...
				if (dbCursor != null) {
					if (dbCursor.getCount() == 0) { // local database is empty, need to sync all data
						// Sync this months database
						if (!syncDB(Db_Helper.DATABASE_NAME_LAST, dbNamesList[1], intentContext)) {
							dataBase.close();
							dbHelper.close();
							return;
						}
	...
	private boolean syncDB(String dbName, String syncMonth, Context intentContext) {
		...
		String solarURL = mPrefs.getString( tk.giesecke.myhomecontrol.Cl_MyHomeControl.deviceNames[ tk.giesecke.myhomecontrol.Cl_MyHomeControl.spMonitorIndex],"NA");
		String urlString = "http://" + solarURL + "/sd/spmonitor/query2.php"; // URL to call

		// Check for last entry in the local database
		/** Instance of Db_Helper */
		Db_Helper dbHelper = new Db_Helper(intentContext, dbName);
		/** Instance of data base */
