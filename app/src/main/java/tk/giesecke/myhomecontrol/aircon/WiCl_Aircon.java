 ...
 */
public class WiCl_Aircon extends IntentService {
		...
		if (intent != null) {
			...
			if (intent.getAction().equals("s")){
				if (!isRunning) {
					...
					if (BuildConfig.DEBUG) Log.d(DEBUG_LOG_TAG, "Second command = " + "c="  + tk.giesecke.myhomecontrol.Cl_MyHomeControl.CMD_OTHER_TIMER);
					if (espComm("t=" + Integer.toString(timerTime))) {
						if (espComm("c=" + tk.giesecke.myhomecontrol.Cl_MyHomeControl.CMD_OTHER_TIMER)) {
							isRunning = true;
					...
					if (BuildConfig.DEBUG) Log.d(DEBUG_LOG_TAG, "First command = " + "c="  + tk.giesecke.myhomecontrol.Cl_MyHomeControl.CMD_OTHER_TIMER);
					if (espComm("c=" + tk.giesecke.myhomecontrol.Cl_MyHomeControl.CMD_OTHER_TIMER)) {
						isRunning = false;
			...
			for (int appWidgetId : appWidgetIds) {
				Wi_Aircon.updateAppWidget(this,appWidgetManager,appWidgetId, timerTime, timerEnd, isRunning);
			}
		}
	}
	...
	private boolean espComm(String cmd) {
		...
		String url = mPrefs.getString(
				tk.giesecke.myhomecontrol.Cl_MyHomeControl.deviceNames[tk.giesecke.myhomecontrol.Cl_MyHomeControl.aircon1Index],
				"NA");
		try {
	 ...
	 * 		Context
	 */
	public static void doPublish(String payload, Context widgetContext){
