...
import tk.giesecke.myhomecontrol.devices.Cl_NsdHelper;
...
import static tk.giesecke.myhomecontrol.Sv_MessageListener.bytesLoadRcvd;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.bytesLoadSend;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.bytesMsgsRcvd;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.bytesMsgsSend;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.clientsConn;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.mqttClientList;
import static tk.giesecke.myhomecontrol.Sv_MessageListener.unSubscribeBrokerStatus;
import static tk.giesecke.myhomecontrol.devices.Cl_NsdHelper.mServicesHosts;

public class Cl_MyHomeControl extends AppCompatActivity implements View.OnClickListener
		, AdapterView.OnItemClickListener , AT_LoadImage.Listener , SeekBar.OnSeekBarChangeListener {
	...
	public static final String sharedPrefName = "Cl_MyHomeControl";
...
//	/** deviceIsOn index for video camera 1 */
//	private static final int vc1Index = 10;
	...
	/** Instance of Db_Helper for this month*/
	private Db_Helper dbHelperNow;
	/** Instance of Db_Helper for last month*/
	private Db_Helper dbHelperLast;
...
//	private static final String[] espIP = {"192.168.0.50",
//			"192.168.0.51",
//			"192.168.0.52",
//			"192.168.0.53",
//			"192.168.0.54",
...
//			"192.168.0.60"};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Open databases
		dbHelperNow = new Db_Helper(appContext, Db_Helper.DATABASE_NAME);
		dbHelperLast = new Db_Helper(appContext, Db_Helper.DATABASE_NAME_LAST);
	...
	protected void onResume() {
		...
		if (myServiceIsStopped(Sv_MessageListener.class)) {
		...
		IntentFilter intentFilter = new IntentFilter(Sv_MessageListener.BROADCAST_RECEIVED);
		//Map the intent filter to the receiver
		registerReceiver(activityReceiver, intentFilter);

		// Register screen on/off broadcast receiver if no longer registered
		if (!BR_Events.eventReceiverRegistered) {
		...
		Sv_MessageListener.uiStarted = true;
		...
		if (thisBundle != null) {
			if  (thisBundle.getInt("view", 99) != 99){
				if (BuildConfig.DEBUG) Log.d(DEBUG_LOG_TAG, "Restart with view: "
						+ thisBundle.getInt("view", 99)
						+ " spm: "
						+ thisBundle.getInt("spm", 99)
						+ " aircon: "
						+ thisBundle.getInt("aircon", 99)
						+ " seccam: "
						+ thisBundle.getInt("seccam", 99));
				// Get the requested view of call
				visibleView = thisBundle.getInt("view", view_security_id);
			}
			if (thisBundle.getInt("spm",99) != 99) {
				visibleView = view_solar_id;
			}
			if (thisBundle.getInt("aircon",99) != 99) {
				visibleView = view_aircon_id;
			}
			if (thisBundle.getInt("seccam",99) != 99) {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case R.id.action_selAlarm:
				...
				int uriIndex = tk.giesecke.myhomecontrol.Cl_Utilities.getNotifSounds(this, notifNames, notifUri, isSelAlarm) + 2;
			...
			case R.id.action_refresh:
				if (tk.giesecke.myhomecontrol.Cl_Utilities.isHomeWiFi(this)) {
					...
					if (myServiceIsStopped(Sv_CheckAvailDevices.class)) {
...
//				break;
						...
						if (!handleLightButtons(v)) { // Check if it was a light control view button and handle it
							switch (v.getId()) { // Handle other buttons right here
								...
								case R.id.dia_sel_device2:
									...
									tk.giesecke.myhomecontrol.Cl_Utilities.highlightDlgIcon(iconButtons[dlgIconIndex], airconDialogView, appContext);
													...
													new DialogInterface.OnClickListener() {
														...
														public void onClick(DialogInterface dialog, int id) {
															...
															if (dlgDeviceIndex == FUJIDENZO) {
																...
																locationIcon.setImageDrawable(getResources().getDrawable(iconIDs[deviceIcon[dlgDeviceIndex]]));
															} else if (dlgDeviceIndex == AMERICANHOME) {
	...
	public void onStopTrackingTouch(SeekBar seekBar) {
		...
		new Cl_MyHomeControl.ESPbyTCP(getString(R.string.LIGHTS_BEDROOM),lightValue,"0");
			...
			if (sender.equalsIgnoreCase("NSD")) {
				...
				if (nsdScanResult) { // Scan finished successful
					...
					for (int i=0; i<deviceNames.length; i++) {
						...
						int listIndex = 0;
						for (int j = 0; j< Cl_NsdHelper.mServicesNames.length; j++) {
							if (deviceNames[i].equalsIgnoreCase(Cl_NsdHelper.mServicesNames[j])) {
					...
					tv.setMaxLines(Cl_NsdHelper.mServicesNames.length+2);
				...
				// Ask for status from light control unit
				new ESPbyTCP(getString(R.string.LIGHTS_BACKYARD), "s", "0");
				return;
			} else {
...
//							break;
			...
			St_CommResult result = new St_CommResult();
				...
				try {
					...
					switch (broadCastDevice) {
						case "spm":
							...
							if (debugDeviceList[spMonitorIndex] || cmdDeviceList[spMonitorIndex]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "fd1":
							...
							if (debugDeviceList[aircon1Index] || cmdDeviceList[aircon1Index]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "ca1":
							...
							if (debugDeviceList[aircon2Index] || cmdDeviceList[aircon2Index]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "am1":
							...
							if (debugDeviceList[aircon3Index] || cmdDeviceList[aircon3Index]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "sf1":
							...
							if (debugDeviceList[secFrontIndex] || cmdDeviceList[secFrontIndex]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "sb1":
							...
							if (debugDeviceList[secBackIndex] || cmdDeviceList[secBackIndex]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
...
//							String leadDigits;
							...
							break;
						case "cm1":
							if (debugDeviceList[cam1Index] || cmdDeviceList[cam1Index]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							result.comCmd = "/?s";
						...
						case "ly1":
							if (jsonResult.has("lo")) {
								ToggleButton byLightSwitch = (ToggleButton) findViewById(R.id.tb_byard_light);
							...
							break;
						case "moni":
							if (debugDeviceList[moniIndex] || cmdDeviceList[moniIndex]) {
								debugMsgs += message + "\n";
								addDebugTxt = true;
							}
							break;
	 ...
	 */
	private class SPMPcommunication extends AsyncTask<String, String, St_CommResult> {
		 ...
		 * 	@return <code>St_CommResult</code>
		 * 			Requester ID and result of communication
		 */
		@Override
		protected St_CommResult doInBackground(String... params) {
...
//			result.deviceIndex = Integer.parseInt(params[4]);
		 ...
		 */
		protected void onPostExecute(St_CommResult result) {
	...
	private class doPublishAsync extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			if (Sv_MessageListener.mqttClient == null) { // If service is not (yet) active, don't publish
			...
			try {
				...
				token = Sv_MessageListener.mqttClient.publish("/CMD", message);
	...
	private class ESPbyTCPAsync extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			try {
				...
				Socket tcpSocket = new Socket(tcpServer, Sv_MessageListener.TCP_CLIENT_PORT);
	...
	private class ESPbyTCP implements Runnable {
		...
		public void run() {
			...
			if (!tk.giesecke.myhomecontrol.Cl_Utilities.isHomeWiFi(getApplicationContext()) && !targetDevice.equalsIgnoreCase("chk")) {
...
//				mqttTopic += "\"cm\":\"" + targetMessage.substring(1) + "\"}"; // The command
	 ...
	 */
	private class galleryCommunication extends AsyncTask<String, String, St_CommResult> {
...
//			if (result.deviceIndex == 0) { // Load latest image and display
			if (!availImages.isEmpty()) {
				...
				new AT_LoadImage((AT_LoadImage.Listener)appContext).execute(lastImage);
			}
...
//			}
	 ...
	 */
	private void securityViewUpdate(final St_CommResult result) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				String tempString;
				if (tk.giesecke.myhomecontrol.Cl_Utilities.isJSONValid(result.comResult)) {
					...
					try {
						...
						if (deviceIDString.equalsIgnoreCase("sf1")) {
							message = tk.giesecke.myhomecontrol.Cl_Utilities.getDeviceStatus(jsonResult, appContext,
									ivAlarmStatus, ivLightStatus,
									secBackView, secAutoAlarmFront, secChangeAlarm);
							message += tk.giesecke.myhomecontrol.Cl_Utilities.getLightStatus(jsonResult);
	 ...
	 */
	private void airconViewUpdate(St_CommResult result) {
...
//			String urlString = "http://" + deviceIPs[spMonitorIndex] + "/sd/spMonitor/query2.php"; // URL to call
	...
	private void solarViewUpdate(final String value, final boolean isBroadCast) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (value.length() != 0) {
					// decode JSON
					if (tk.giesecke.myhomecontrol.Cl_Utilities.isJSONValid(value)) {
						...
						try {
							...
							try {
								Cl_ChartHelper.solarPowerMin = isFromLocal?
										Float.parseFloat(jsonValues.getString("S")):
										Float.parseFloat(jsonValues.getString("s"));
								Cl_ChartHelper.lastSolarPowerMin = Cl_ChartHelper.solarPowerMin;
							} catch (Exception excError) {
								Cl_ChartHelper.solarPowerMin = Cl_ChartHelper.lastSolarPowerMin;
							}
							try {
								Cl_ChartHelper.consPowerMin = isFromLocal?
										Float.parseFloat(jsonValues.getString("C")):
										Float.parseFloat(jsonValues.getString("c"));
								Cl_ChartHelper.lastConsPowerMin = Cl_ChartHelper.consPowerMin;
							} catch (Exception excError) {
								Cl_ChartHelper.consPowerMin = Cl_ChartHelper.lastConsPowerMin;
							}

							result = "S=" + String.valueOf(Cl_ChartHelper.solarPowerMin) + "W ";
							result += "s=";
							try {
							...
							result += "\nC=" + String.valueOf(Cl_ChartHelper.consPowerMin) + "W c=";
							try {
							...
							double resultPowerMin = Cl_ChartHelper.solarPowerMin + Cl_ChartHelper.consPowerMin;

							valueFields = (TextView) findViewById(R.id.tv_solar_val);
							/** String for display */
							String displayTxt;
							displayTxt = String.format("%.0f", Cl_ChartHelper.solarPowerMin) + "W";
							...
							if (Cl_ChartHelper.consPowerMin > 0.0d) {
							...
							displayTxt = String.format("%.0f", Math.abs(Cl_ChartHelper.consPowerMin)) + "W";
							valueFields.setText(displayTxt);

							if (Cl_ChartHelper.autoRefreshOn) {
								if (Cl_ChartHelper.plotData != null) {
									/** Current time as string */
									String nowTime = tk.giesecke.myhomecontrol.Cl_Utilities.getCurrentTime();
									Cl_ChartHelper.plotData.addXValue(nowTime);
									Cl_ChartHelper.timeStampsCont.add(nowTime);
									Cl_ChartHelper.solarSeries.add
											(new Entry(Cl_ChartHelper.solarPowerMin, Cl_ChartHelper.solarSeries.size()));
									Cl_ChartHelper.solarPowerCont.add(Cl_ChartHelper.solarPowerMin);
									if (Cl_ChartHelper.consPowerMin < 0.0) {
										Cl_ChartHelper.consPSeries.add
												(new Entry(Cl_ChartHelper.consPowerMin, Cl_ChartHelper.consPSeries.size()));
										Cl_ChartHelper.consumPPowerCont.add(Cl_ChartHelper.consPowerMin);
										Cl_ChartHelper.consMSeries.add(new Entry(0, Cl_ChartHelper.consMSeries.size()));
										Cl_ChartHelper.consumMPowerCont.add(0.0f);
									} else {
										Cl_ChartHelper.consMSeries.add
												(new Entry(Cl_ChartHelper.consPowerMin, Cl_ChartHelper.consMSeries.size()));
										Cl_ChartHelper.consumMPowerCont.add(Cl_ChartHelper.consPowerMin);
										Cl_ChartHelper.consPSeries.add(new Entry(0, Cl_ChartHelper.consPSeries.size()));
										Cl_ChartHelper.consumPPowerCont.add(0.0f);
									}
									...
									displayTxt = "(" + String.format("%.0f",
											Collections.max(Cl_ChartHelper.consumMPowerCont)) + "W)";
									maxPowerText.setText(displayTxt);
									maxPowerText = (TextView) findViewById(R.id.tv_solar_max);
									displayTxt = "(" + String.format("%.0f",
											Collections.max(Cl_ChartHelper.solarPowerCont)) + "W)";
									maxPowerText.setText(displayTxt);

									// let the chart know it's data has changed
									Cl_ChartHelper.lineChart.notifyDataSetChanged();
									Cl_ChartHelper.lineChart.invalidate();
								}
							}
						} catch (JSONException e) {
							e.printStackTrace();
						...
						result += tk.giesecke.myhomecontrol.Cl_Utilities.getCurrentTime();
						solStatus.setText(result);
					}
				} else {
	...
	private class mqttDebugAsync extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			if (task.equalsIgnoreCase("subscribe")) {
				...
				Sv_MessageListener.subscribeBrokerStatus();
			} else {
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (!showingLog) {
					...
					int[] todayDate = tk.giesecke.myhomecontrol.Cl_Utilities.getCurrentDate();
					/** Array with existing log dates on the Arduino */
					...
					try {
						...
						Cursor newDataSet = Db_Helper.getDay(dataBase, todayDate[2],
								todayDate[1], todayDate[0] - 2000);
						Cl_ChartHelper.fillSeries(newDataSet, appView);
						if (newDataSet != null) {
							newDataSet.close();
						}
						thisLogDates.clear();
						...
						for (int year = 0; year < yearsAvail.size(); year++) {
							...
							ArrayList<Integer> monthsAvail = Db_Helper.getEntries(dataBase, "month",
									0, yearsAvail.get(year));
							for (int month = 0; month < monthsAvail.size(); month++) {
								/** List with days of month of year in the database */
								ArrayList<Integer> daysAvail = Db_Helper.getEntries(dataBase, "day",
						...
						if (syncMonth.equalsIgnoreCase(dbNamesList[0])) {
							...
							Cl_ChartHelper.initChart(true, thisAppContext, chartTitle);
						} else {
	...
	private void setGlobalVar() {
		...
		Cl_ChartHelper.lineChart = (LineChart) findViewById(R.id.graph);
		...
		airAMView = (RelativeLayout) findViewById(R.id.american);
		...
		btTimerAM = (Button) findViewById(R.id.bt_timer_am);

		btFanCA = (Button) findViewById(R.id.bt_fanspeed_ca);
		btFanAM = (Button) findViewById(R.id.bt_fanspeed_am);
		...
		txtConsValAM = (TextView) findViewById(R.id.txt_cons_val_am);
		txtTempValAM = (TextView) findViewById(R.id.txt_temp_val_am);
		txtAutoStatusValAM = (TextView) findViewById(R.id.txt_auto_status_val_am);
	...
	private void initSPM() {
		if (deviceIsOn[spMonitorIndex]) { // spMonitor
			...
			dbNamesList = tk.giesecke.myhomecontrol.Cl_Utilities.getDateStrings();
	...
	private void switchUI(int uiSelected) {
		...
		switch (uiSelected) {
			...
			default:
				...
				visibleView = view_solar_id;
//				if (Cl_ChartHelper.autoRefreshOn) {
//					Cl_ChartHelper.initChart(true, appContext, chartTitle);
//				} else {
//					Cl_ChartHelper.initChart(false, appContext, chartTitle);
//				}
				Cl_ChartHelper.initChart(Cl_ChartHelper.autoRefreshOn, appContext, chartTitle);
				break;
		}
	...
	private boolean handleSecurityButtons(View v) {
		...
		switch (v.getId()) {
			...
			case R.id.tv_change_alarm:
				final Dialog alarmDlg = new Dialog(Cl_MyHomeControl.this);
			...
			case R.id.ib_snapshot_gallery2:
				...
				break;
			case R.id.ib_seccam:
				Intent myIntent = new Intent(this, Cl_SecCamViewer.class);
		...
		switch (v.getId()) {
			case R.id.bt_prevLog:
				...
				if (!showingLast) { // use this months database
					if (logDatesIndex > 0) {
						...
						Cl_ChartHelper.autoRefreshOn = false;
						showingLog = true;
						...
						/** Cursor with new data from the database */
						Cursor newDataSet = Db_Helper.getDay(dataBase, Integer.parseInt(requestedDate[2]),
								Integer.parseInt(requestedDate[1]), Integer.parseInt(requestedDate[0]));
						Cl_ChartHelper.fillSeries(newDataSet, appView);
						Cl_ChartHelper.initChart(false, thisAppContext, chartTitle);
						newDataSet.close();
						dataBase.close();
			...
			case R.id.bt_stop:
				if (Cl_ChartHelper.autoRefreshOn) {
					...
					Cl_ChartHelper.autoRefreshOn = true;
				}
				break;
			default:
				wasSPMbutton = false;
