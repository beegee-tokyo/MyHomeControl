...
uint32_t Adafruit_VC0706::autoDetectBaudRate() {
	if (begin(38400)) {
    Serial.println("Found camera with 38400");
    return 38400;
  }
  delay(1000);
  if (begin(9600)) {
    Serial.println("Found camera with 9600");
    return 9600;
  }
  delay(1000);
  if (begin(19200)) {
    Serial.println("Found camera with 19200");
    return 19200;
  }
  delay(1000);
  if (begin(57600)) {
    Serial.println("Found camera with 57600");
    return 57600;
  }
  delay(1000);
  if (begin(115200)) {
    Serial.println("Found camera with 115200");
    return 115200;
  } else {
