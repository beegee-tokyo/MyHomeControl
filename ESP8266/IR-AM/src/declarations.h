...
/** Mode status of aircon (only guess, as human could have used remote)
		...
		tttttt = temperature:
								...
								100000 32 deg C
*/
/** The American Home aircon stores temperature and fanspeed for each
 mode separate, we need to remind these values for each mode */
long acModeAuto =		Mode_Auto;

long acModeCool =		Mode_Cool;

long acModeDry =		Mode_Dry;

long acModeFan =		Mode_Fan;

/** Values for current mode */
long acModeCurr =		Mode_Auto;
/** Current mode 0 = Auto, 1 = Cool, 2 = Dry, 3 = Fan */
byte currAcMode = 0;
