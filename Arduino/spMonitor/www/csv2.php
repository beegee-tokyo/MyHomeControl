...
$outFile = fopen("/mnt/sda1/".substr($dateSelect,0,5).".csv",'w')
	or die("Can't open /mnt/sda1/".substr($dateSelect,0,5).".csv");
fputcsv($outFile, array('id','d','s','c','l'));
foreach($json as $data){
    fputcsv($outFile, $data);
}
fclose($outFile) or die("Can't close /mnt/sda1/".substr($dateSelect,0,5).".csv");
echo "<p>File saved</p>";
echo "</font></body></html>";
